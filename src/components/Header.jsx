...
import { 
  ...
  CheckSquare, Award, Heart, Star, FileText 
} from 'lucide-react';
import { playHangeulSound } from '../utils/speech';

/**
 * [상단 헤더 - 메뉴 텍스트 1줄 정돈 및 반응형 네비게이션]
 * 메뉴 탭 텍스트가 줄바꿈되어 두 줄로 어색하게 깨지지 않도록
 * 직관적이고 간결한 1줄 명칭(whitespace-nowrap)으로 최적화하였습니다.
 */
        ...
        <div className="flex items-center justify-between w-full md:w-auto shrink-0">
          ...
          <div className="md:hidden flex items-center gap-1.5 shrink-0">
            <button
              ...
              className="flex items-center gap-1 bg-amber-100 hover:bg-amber-200 border border-amber-300 px-2.5 py-1 rounded-full text-xs font-black text-amber-950 whitespace-nowrap"
              title="학생 변경"
            >
              <span>{currentStudent?.avatar || '🐯'}</span>
              <span>{currentStudent?.name || '꿈나무'}</span>
            </button>
            <div className="flex items-center gap-1 bg-amber-100 px-2.5 py-1 rounded-full font-black text-amber-900 text-xs whitespace-nowrap">
